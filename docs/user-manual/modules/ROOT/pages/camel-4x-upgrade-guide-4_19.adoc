...
The consumer now supports SQL++ (N1QL) queries as an alternative to the deprecated MapReduce Views.
Use the new `statement` endpoint option to specify a SQL++ query. When `statement` is set, the consumer
uses `scope.query()` instead of `bucket.viewQuery()`.
...
Couchbase Server 8.0). Users should migrate to SQL++ queries using the `statement` option.
